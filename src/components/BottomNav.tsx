...
export default function BottomNav() {
  ...
  return (
    <div className="md:hidden fixed bottom-6 left-0 right-0 z-50 flex justify-center px-4 pointer-events-none">
      <nav className="pointer-events-auto bg-white/85 dark:bg-black/60 backdrop-blur-2xl border border-black/5 dark:border-white/10 shadow-[0_8px_30px_rgb(0,0,0,0.12)] rounded-full px-6 py-2 flex items-center space-x-3 transition-all">
        {tabs.map((tab) => {
          const currentContext =
            pathname.startsWith("/play") || pathname.startsWith("/video")
              ? activeRoot
              : pathname;
          const isActive =
            tab.href === "/"
              ? currentContext === "/"
              : currentContext.startsWith(tab.href);
          ...
          return (
            <Link
              ...
              className={`flex flex-col items-center justify-center transition-all duration-300 ease-out py-1.5 rounded-full ${isActive && !isBackToTop ? "px-8 bg-[#ededed] dark:bg-[#1e2939] text-[#1e2939] dark:text-white shadow-sm" : isBackToTop ? "px-8 bg-black/80 dark:bg-white/90 text-white dark:text-black shadow-lg scale-110" : "px-5 text-gray-600 dark:text-gray-500 hover:text-[#1e2939] dark:hover:text-white"}`}
            >
