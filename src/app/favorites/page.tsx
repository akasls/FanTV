...
export default function FavoritesPage() {
  const { currentMode, favoriteData, setFavoriteData, historyData } = useAppStore()
  const [search, setSearch] = useState('')

  const displayData = useMemo(() => {
     let data = favoriteData.filter(v => v.mode === currentMode)
     if (search.trim()) {
    ...
    <div className="space-y-3 pb-24">
      ...
      <div className="relative mb-6">
        ...
        <input
          ...
          className="w-full bg-white dark:bg-[#1c1c1e] text-gray-900 border-none dark:text-white rounded-full pl-11 pr-4 py-3.5 focus:outline-none focus:ring-2 focus:ring-red-500/50 shadow-sm transition-shadow"
          placeholder="在收藏中搜索..."
        />
      </div>
      ...
      {displayData.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20">
          <div className="w-20 h-20 mb-4 rounded-full bg-red-50 dark:bg-red-900/20 flex items-center justify-center">
            <svg className="w-10 h-10 text-red-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
            </svg>
          </div>
          <p className="text-gray-500 font-medium">暂无收藏记录</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 sm:gap-5 md:gap-6 lg:gap-8">
          {displayData.map((video, idx) => {
             const hist = historyData.find(h => h.videoId === String(video.videoId) && h._sourceId === video._sourceId);
                  ...
                  <div className="absolute inset-x-0 bottom-0 pt-16 pb-3 px-3">
                    ...
                    {hist?.epName && (
                      <p className="relative text-xs text-gray-300 mt-1 flex items-center justify-between">
                        <span className="truncate">{hist.epName}</span>
                      </p>
                    )}
                    {(hist?.duration || 0) > 0 && (hist?.currentTime || 0) > 0 && (
                      <div className="relative mt-2 h-1 w-full bg-white/20 rounded-full overflow-hidden">
                        <div className="absolute top-0 left-0 h-full bg-blue-500" style={{ width: `${Math.min(100, ((hist!.currentTime || 0) / (hist!.duration || 1)) * 100)}%` }} />
                      </div>
                    )}
                  </div>
